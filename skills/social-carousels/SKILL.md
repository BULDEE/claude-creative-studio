...
- Zero internal margins on text frames (pixel-perfect positioning)
- Explicit line spacing on every paragraph
- Letter spacing on mono labels
- Rounded rectangles for cards and comparison rows

**Import options:**
- **Canva**: import the PPTX directly, adjust text and layout
- **Gamma**: import PPTX → customize → save as template → use API `from-template` for scale
- **Figma**: use the slides/ PNG folder + carousel.json for Figma frame specs

#### Step 3 (optional): Generate PNG screenshots

Use Playwright to screenshot the HTML slides for social media posting:
...
- Creating generation scripts (.py, .mjs) in the client's project — use plugin scripts
- Leaving draft files or temporary assets in the carousel folder
</avoid>
